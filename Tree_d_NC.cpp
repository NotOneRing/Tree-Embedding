...
int main(int argc,  char **argv){
  ...
  for(int iter = 1; iter < snapshots_number; iter++){
    ...
    for(int i = 0; i < count_labeled_node; i++){
      delete []flags_transpose[i];
      delete []queue_list_transpose[i].arr;
    }

    delete []flags_transpose;
    delete []queue_list_transpose;
    ...
    U_all_shots.block(iter_beginning + start, 0, row_number, d) = U_Eigen;
