...
double matrix_get_element(mat *M, int row_num, int col_num){
    ...
    // if(col_num*(M->nrows) + row_num >= M->ncols * M->nrows){
    //     printf("row_num = %d\n", row_num);
    //     printf("col_num = %d\n", col_num);
    //     printf("M->nrows = %d\n", M->nrows);
    //     printf("M->ncols = %d\n", M->ncols);
...
/* subtract B from A and save result in A  */
void matrix_sub_store(mat *A, mat *B, mat* C){
    int i;
    //#pragma omp parallel for
...
void matrix_transpose_matrix_mult(mat *A, mat *B, mat *C){
    double alpha, beta;
    alpha = 1.0; beta = 0.0;
    // printf("matrix_transpose_matrix_mult\n");
...
void matrix_matrix_transpose_mult(mat *A, mat *B, mat *C){
    double alpha, beta;
    alpha = 1.0; beta = 0.0;
    // printf("matrix_matrix_transpose_mult\n");
...
void QR_factorization_getQ_inplace(mat *Q) {
    ...
    if(jpvt == NULL){
        printf("jpvt allocation failure!!!\n");
    }    
...
/* computes SVD: M = U*S*Vt; note Vt = V^T */
void truncated_singular_value_decomposition(mat *M, mat *U, mat *S, mat *Vt, int d){
    int m,n,k;
    ...
    mat *UU = matrix_new(M->nrows, M->ncols);

    mat *SS = matrix_new(M->ncols, M->ncols);

    mat *VV = matrix_new(M->ncols, M->ncols);


    LAPACKE_dgesvd( LAPACK_COL_MAJOR, 'S', 'S', m, n, M->d, m, svals->d, UU->d, m, VV->d, k, work->d );


    initialize_diagonal_matrix(SS, svals);


    for(int i = 0; i < M->nrows; i++){
        for(int j = 0; j < d; j++){
            matrix_set_element(U, i, j, matrix_get_element(UU, i, j));
        }
    }
    ...
    for(int i = 0; i < d; i++){
        for(int j = 0; j < d; j++){
            matrix_set_element(S, i, j, matrix_get_element(SS, i, j));
        }
    }
    ...
    for(int i = 0; i < d; i++){
        for(int j = 0; j < M->ncols; j++){
            matrix_set_element(Vt, i, j, matrix_get_element(VV, i, j));
        }
    }
...
void QR_Decomposition_original(mat *A){
    int m,n,i,j,ind;
    ...
    for(i=0; i<n; i++){
        ...
        for(j=0; j<i; j++){
            matrix_get_col(Q, j, vj);
            double r = vector_dot_product(vi, vj);
        ...
        double norm = vector_get2norm(vi);

        if(norm < EPS){
            for(int k = i; k < n; ++k){
                vec* vk = vector_new(m);
                matrix_get_col(Q, k, vk);
                vector_scale(vk, 0);
                return;                
            }
        }

        vector_scale(vi, 1.0/vec_norm);
        matrix_set_col(Q, i, vi);
    }
...
void set_matrix_zeros(mat *A){
    int rows = A->nrows;
    int cols = A->ncols;
    for(int i=0; i<rows; i++){
        for(int j=0; j<cols; j++){
            matrix_set_element(A, i, j, 0);
        }
    }
}
...
void set_matrix_ones(mat *A){
    int rows = A->nrows;
    int cols = A->ncols;
    for(int i=0; i<rows; i++){
        for(int j=0; j<cols; j++){
            matrix_set_element(A, i, j, 1);
        }
    }
}
...
void set_matrix_identity(mat *A){
    int rows = A->nrows;
    int cols = A->ncols;
    for(int i=0; i<rows; i++){
        for(int j=0; j<cols; j++){
...
block_set_value(mat* receive_block, int start_row, int start_col, int rows, int cols, mat* value_block){
    for(int i = 0; i < rows; i++){
        for(int j = 0; j < cols; j++){
            double element = matrix_get_element(value_block , i, j);
            matrix_set_element(receive_block, start_row + i, start_col + j, element);
        }
    }
}
